        ...
        this.container.innerHTML = `
            <div class="calendar-widget">
                <div class="calendar-header">
                    <button class="calendar-nav-btn prev" onclick="calendar.prevMonth()">←</button>
...
// ===== GLOBAL SWIPE PROTECTION =====
// Prevents the native browser "White Back Arrow" on edge swipes
function preventGlobalSwipeBack() {
    let touchStartX = 0;
    let touchStartY = 0;
    ...
    document.addEventListener('touchmove', (e) => {
        ...
        // Condition 1: Must be a swipe RIGHT (positive X diff)
        // Condition 2: Must be starting near the LEFT EDGE (0-30px) where back gestures trigger
        // Condition 3: Must be MORE Horizontal than Vertical
        if (touchStartX < 40 && diffX > 5 && Math.abs(diffX) > Math.abs(diffY)) {
            if (e.cancelable) {
                e.preventDefault();
                // console.log('🚫 Native Back Swipe Blocked');
            }
        }
    }, { passive: false });

    console.log('🛡️ Global swipe protection active');
}
...
function initFloatingChatWidget() {
    ...
    function addBotMessage(text) {
        ...
        // After 1.5-2 seconds, remove typing and show actual message
    ...
    chatFab.addEventListener('click', () => {
        ...
        // Hide header and nav bar for fullscreen experience
        ...
        if (isOpen) {
            ...
            // Hide the FAB button itself when chat is open
            if (floatingWidget) floatingWidget.querySelector('.chat-fab').style.display = 'none';

            if (chatMessages.children.length === 0) {
                setTimeout(() => addBotMessage(botResponses.greeting), 300);
            }
        } else {
            if (mainHeader) mainHeader.style.display = '';
            if (bottomNav) bottomNav.style.display = '';
            if (floatingWidget) floatingWidget.querySelector('.chat-fab').style.display = '';
        }
    });
...
function initServicesPage() {
    ...
    servicesGridContainer.addEventListener('touchmove', (e) => {
        ...
        if (diffX > 15) {
            // STOP NATIVE BROWSER BACK NAVIGATION
            if (e.cancelable) {
                e.preventDefault();
            }
