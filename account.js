...
class AccountController {
    ...
    setupProfileListeners() {
        const openBtn = document.getElementById('openProfileDetailsBtn');
        const closeBtn = document.getElementById('closeProfileBtn');
        const modal = document.getElementById('profileDetailsModal');

        if (openBtn) {
            openBtn.onclick = () => this.openProfileDetails();
        }
        ...
        if (editBtn) editBtn.onclick = () => this.enableEditMode(true);
    ...
    async openProfileDetails() {
        ...
        if (this.currentUser) {
            ...
            this.fetchExtendedProfile(this.currentUser.uid);
        }
    }

    populateProfileForm(user) {
        document.getElementById('detailProfileImage').src = user.photoURL || 'assets/default-avatar.png';
        document.getElementById('detailProfileNameDisplay').textContent = user.displayName || 'Guest';
        document.getElementById('detailProfileEmailDisplay').textContent = user.email || '';

        document.getElementById('detailFullName').value = user.displayName || '';
        document.getElementById('detailEmail').value = user.email || '';
        document.getElementById('detailPhone').value = user.phoneNumber || ''; // Phone might be empty if email auth used
    }

    async fetchExtendedProfile(uid) {
        if (!this.db) return;
        ...
        try {
            ...
            if (doc.exists) {
                const data = doc.data();
                if (data.address) document.getElementById('detailAddress').value = data.address;
                if (data.bio) document.getElementById('detailBio').value = data.bio;
                if (data.gender) document.getElementById('detailGender').value = data.gender;
                if (data.dob) document.getElementById('detailDob').value = data.dob;

                if (data.phoneNumber && !this.currentUser.phoneNumber) {
                    // Fallback if auth doesn't have phone but DB does
    ...
    enableEditMode(enable) {
        const inputs = ['detailFullName', 'detailAddress', 'detailBio', 'detailGender', 'detailDob'];
        const editBtn = document.getElementById('enableEditBtn');
        const saveBtn = document.getElementById('saveProfileBtn');
        ...
        if (saveBtn) saveBtn.classList.toggle('hidden', !enable);
        if (photoBtn) photoBtn.classList.toggle('hidden', !enable);
    }

    async saveProfileChanges() {
        const newName = document.getElementById('detailFullName').value;
        const newAddress = document.getElementById('detailAddress').value;
        const newBio = document.getElementById('detailBio').value;
        ...
        try {
            ...
            if (newName !== this.currentUser.displayName) {
                await this.currentUser.updateProfile({ displayName: newName });
                // Update main account view immediately
                this.renderUserProfile(this.currentUser);
                document.getElementById('detailProfileNameDisplay').textContent = newName;
            }

            // 2. Update Firestore (Address, Bio, Gender, DOB)
            if (this.db) {
            ...
            if (window.showError) window.showError('Failed to save changes');
            else alert('Failed to save changes');
        } finally {
            saveBtn.textContent = 'Save Changes'; // Reset text (icon is in HTML)
